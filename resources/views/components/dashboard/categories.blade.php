<div x-data="{ showModal: false }" class="px-4 py-4 bg-red-300/50 text lg basis-3/12 min-h-screen space-y-3 rounded">
    <div class="flex justify-between items-center ">
        <h1>Categories</h1>
        <a @click="showModal = true" class="hover:text-gray-400 cursor-pointer">+</a>
    </div>
    ...
    {{ request('selectedCategory') == $category->id ? 'bg-red-700/50 hover:bg-red-800/50' : 'bg-red-400/50 hover:bg-red-500/50' }}">
                ...
                <form action="{{ route('categories.destroy', $category) }}" method="POST"
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-white hover:text-red-500 focus:outline-none"
    ...
    @include('components.categories.create')
</div>
